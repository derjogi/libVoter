...
import { electionConfig, type ElectionConfig } from '../src/lib/config/election';
...
async function testElectionConfig() {
  ...
  try {
    ...
    if (electionConfig && typeof electionConfig === 'object') {
      console.log(`✅ Election config loaded: ${electionConfig.location}, ${electionConfig.year}`);
      testsPassed++;
    } else {
  ...
  try {
    ...
    if (electionConfig.location === expectedLocation &&
        electionConfig.year === expectedYear &&
        electionConfig.type === expectedType) {
      console.log(`✅ Election values match: ${electionConfig.location} ${electionConfig.year} - ${electionConfig.type}`);
      testsPassed++;
    } else {
      console.log('❌ Election values do not match expected values');
      console.log(`   Expected: ${expectedLocation}, ${expectedYear}, ${expectedType}`);
      console.log(`   Actual: ${electionConfig.location}, ${electionConfig.year}, ${electionConfig.type}`);
    }
  } catch (error) {
  ...
  try {
    ...
    const result = await promptManager.executePrompt('CANDIDATE_MATCHING', testVariables);
  ...
  try {
    ...
    const hasKeyTopics = electionConfig.keyTopics.every(topic => systemMessage.includes(topic));
  ...
  try {
    ...
    const hasElectionYearVar = candidateMatchingPrompt.template.includes('{electionYear}');
    const hasElectionLocationVar = candidateMatchingPrompt.template.includes('{electionLocation}');
    const hasElectionTypeVar = candidateMatchingPrompt.template.includes('{electionType}');
    const hasElectionKeyTopicsVar = candidateMatchingPrompt.template.includes('{electionKeyTopics}');

    if (hasElectionYearVar && hasElectionLocationVar && hasElectionTypeVar && hasElectionKeyTopicsVar) {
      ...
      console.log('Template preview:', candidateMatchingPrompt.template.substring(0, 300) + '...');
    }
  } catch (error) {
  ...
  if (testsPassed === totalTests) {
    console.log('🎉 All tests passed! Election configuration is working correctly.');
    console.log('✅ The AI now knows it\'s specifically for Auckland 2025 elections.');
  } else {
    console.log('⚠️  Some tests failed. Please check the election configuration.');
  }
