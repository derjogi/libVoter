...
import { pickMockResponse, MOCK_RESPONSES } from '@/lib/server/ai/__mocks__/responses';
import { parseComponentSpec, parseQuestionResponse } from '@/types/components.zod';
...
describe('MockChatModel.invoke', () => {
  it('returns the COMPONENT_SELECTOR fixture when given the selector prompt', async () => {
    ...
    const reply = await model.invoke([
      ...
      { role: 'user', content: 'Available component types and their EXACT data structures: ...' },
    ]);
  ...
  it('returns the FOLLOWUP_QUESTION fixture for follow-up prompts', async () => {
    const model = new MockChatModel();
    const reply = await model.invoke([
      { role: 'user', content: 'Generate a thoughtful follow-up question for the user.' },
    ]);
  ...
  it('returns the EXPLAIN_MATCH fixture for match-explanation prompts', async () => {
    const model = new MockChatModel();
    const reply = await model.invoke([
      { role: 'user', content: 'Provide a clear, balanced explanation of this match.' },
    ]);
  ...
  it('falls back to a generic chat reply for unknown prompts', async () => {
    const model = new MockChatModel();
    const reply = await model.invoke([{ role: 'user', content: 'hello there' }]);
