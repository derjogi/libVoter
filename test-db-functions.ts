import { getUniqueWards, getCandidatesByWard, getMayorCandidates } from './src/lib/actions/database';
...
async function testDatabaseFunctions() {
  ...
  try {
    ...
    if (mayorsResult.success && mayorsResult.data) {
      console.log('✅ getMayorCandidates success:', mayorsResult.data.length, 'candidates');
    ...
    if (wardsResult.success && wardsResult.data && wardsResult.data.length > 0) {
      ...
      if (wardCandidatesResult.success && wardCandidatesResult.data) {
        console.log(`✅ getCandidatesByWard success: ${wardCandidatesResult.data.length} candidates in ${testWard}`);
        if (wardCandidatesResult.data.length > 0) {
          console.log('First candidate:', wardCandidatesResult.data[0].name);
        }
      } else {
        console.log('❌ getCandidatesByWard failed:', wardCandidatesResult.error);
      }
    }
